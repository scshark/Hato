...
func (s *tweetManageServant) CreateCrawlerPost(post *model.Post, content []*model.PostContent) (*model.Post, error) {
	...
	db := s.db.Begin()
	// create post
	hPost, err := post.Create(db)
	if err != nil {
		db.Rollback()
		return nil, errors.New(fmt.Sprintf("crawler post create err %s ", err))
	}
	s.cacheIndex.SendAction(core.IdxActCreatePost, post)

	for _, c := range content {
		c.PostID = hPost.Model.ID
		_, err = c.Create(db)
		if err != nil {
			db.Rollback()
			return nil, errors.New(fmt.Sprintf("crawler post content create err %s ", err))
		}
	}
	db.Commit()
...
func (s *tweetServant) GetUserPostStars(userID int64, offset, limit int) ([]*model.PostStar, error) {
	...
	return star.List(s.db, nil, offset, limit)
}
